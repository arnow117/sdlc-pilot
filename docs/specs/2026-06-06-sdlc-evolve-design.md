...
```
探对 upstream 的推送权:
 ├ 有(owner)   → 开分支 + push + gh pr create(默认也走 PR,不直推 main;
 │                main 始终受保护、有可审查/可被 CI 卡的记录)
 └ 无(第三方)  → push 到自己的 fork + gh pr create 回 upstream
```
- **无 `gh` 的环境**(纯 Codex):降级为打印"手动建 PR"的步骤与 URL,不假装已提。
- 默认 owner 也走 PR(对齐"自我修改要可审查");如确需直推 main,留作显式选项(待评审定)。
...
- owner 直推 main 快路 —— Why:个人快迭代;Trigger:PR 流被证明太重时;Breadcrumbs:§7。
