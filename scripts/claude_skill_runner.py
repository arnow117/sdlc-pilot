...
def _prompt(request: Mapping[str, object], context_pack: str) -> str:
    ...
    return "\n".join([
        ...
        "For a normal routed phase, artifacts must include the exact kind of every selected phase output_contract in the policy surface. Never substitute an input_contract kind or a non-preview canonical artifact (such as EngineeringSpec, DeliveryPlan, or EvidenceSet) for a required preview output.",
