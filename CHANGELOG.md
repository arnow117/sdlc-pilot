...
- 同一执行者负责实现、必要测试和修正；结果必须带实际命令与 exit 结果、revision、workspace 状态和未解项。两轮未验收后先诊断合同或环境；
  两轮尝试既无验收进展也无新定位证据，或两轮无依据重复派工、评审、检查，则停止当前交付段，保留代码和恢复记录，等待用户明确指示后再继续。
...
- 将旧 PR #5 的有效规则迁移到当前验证流程：询问前定向查找相关资料，区分实现/验收失败、环境阻塞与证据不足，避免无依据返工。
- 归因和脱敏环境证据保存在现有工程上下文；必要重验未通过时失效旧通过记录，保留 lifecycle schema 与授权边界。

## [1.7.0] — 2026-09-13

### Added

- 状态查询只读且不初始化生命周期；未跟踪的小型局部可逆修改可直接执行并做必要验证。明确完整流程、已有跟踪任务及跨端、协议、持久化或并发恢复风险仍保留 SDLC 阶段。

### Changed

- 阶段内自主完成实现与验证，集中反馈；减少无新增信息的消息和状态查询，默认使用 60 秒可中断等待。
- 独立 Feature 使用精简上下文，同阶段修正复用 agent；只在相关代码、配置与环境一致且证据完整时复用验证结果。
- 已授权连续完成的工作不新增阶段审批；状态唯一写入者、必要验证、独立评审及版本一致性要求保持不变。公共命令与状态 schema 无变化，无需迁移。

## [1.6.0] — 2026-09-05

### Added

- `sdlc-review` 每次检查本次 diff 中的重复实现、不必要抽象及冗余配置或依赖；建议必须给出删除依据或候选实现证据、行为与工程约束保持依据及验证方式。

### Changed

- 工程债发现与现有角色结论统一去重和分级；普通简化作为建议，有据的保留或延期记录理由与重访条件，采纳后的业务代码修改仍需重新 build、validate、review。
- README 补充使用说明，蒸馏源地图记录 Ponytail 的借鉴范围与取舍。公共命令、状态 schema 与运行时依赖保持不变，无需迁移。
...
- `sdlc-plan` 新增编码前复用评估：依次检查现有代码、当前依赖和官方维护方案，必要时再比较外部候选；研发上下文记录复用、扩展、引入依赖或自研的决定。

### Changed

- `sdlc-build` 对新规划的非简单 Feature 检查复用决定，避免在实现阶段临时引入框架；小型局部修改允许记录不适用，不增加生命周期状态或新的顶层 skill。
...
### Fixed
- 产品设计对齐现在只对会改变产品定位、V1 范围、职责边界或风险接受范围的问题逐题澄清；核心边界稳定后，批量给出推荐默认值并只让用户纠正例外。
- 明确 ProductContract、项目领域评估规格和 EngineeringSpec 的问题归属，避免把企业特定阈值或工程实现细节变成连续产品单选。
- 已接受的同一动作不再重复确认；正式 Approval 仍按 typed record 与授权规则处理。
