...
**可选 · 网页划词批注复核**：plan 阶段多、任务多、聊天逐条对太累时，可把 plan.md 渲染成「划词加批注」的本地网页让用户标注，再统一改回——见 `references/web-review/playbook.md`。**主动提醒**:向用户呈现定稿 plan 请其确认时,**顺带提一句**"plan 长/任务多的话可说『网页审』改用划词批注"——别让用户自己去发现有这条路;简单 plan 可省,默认聊天确认更快。它只升级"意见怎么喂回来"的机制，不改"最终 plan 确认"的纪律。默认走 text_mode 聊天确认；用户说"网页审/划词批注"才用。需要**实时双向**（提交即自动回流、改完页面自刷新、跨引擎）→ 同 playbook §6 Live mode。
...
## 8. 写什么进 STATE（handoff，单写者）

plan 定稿、双向追溯全过后，**经由 driver** 写回 `.sdlc/STATE.md`（单写者；并行产物各写各文件）：
...
9. [ ] 经 driver 写回 `STATE.md`：gates / roles / modes / changed-files / decisions / next（§8）
