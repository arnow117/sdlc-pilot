...
>
> distilled-from: `session:loop-engineering-article(Addy Osmani)` · `kb-manage`(递归 domain-subdomain + Ingest) · `tb-loop-driver`(导演/编排模式) · `session:sdlc-backlog-build-2026-06-15` · `session:sdlc-feature-retirement-2026-06-16`(Retire op / 特性退场闭环)
...
**终态 `shipped` 回写**由本 skill 的 **Retire 操作**(§5,特性 done 时)负责——填上原先悬空的回写点,
...
## 5. 操作: Retire(特性退场 / close-out)

**何时**:一个单特性走到 `stage=done`。由 **driver** 在 `/sdlc` 入口检测到 `STATE.stage==done` 时路由进来(见 sdlc driver §2/§4),给完成的特性收尾。这是 backlog 在生命周期**末端**的职责——与"选 ready 叶起特性"(§5 出口的开端)首尾呼应。
