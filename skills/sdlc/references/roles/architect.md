...
- **spec(§2.5 设计)**:审本特性的端到端数据流与跨边界契约,数据结构在设计阶段就对齐(别等实现完才发现两头对不上)。普通 feature 只做简短的反例检查；仅当要定稿持久化格式、跨机器协调、不可逆外部行为或公共 API 兼容协议时，才按 `doubt-driven-development` 做一次对抗性复审。
