...
build 红 / converge 不通过 / 调试失败时,**先写一行根因教训**(经 DEBUG REPORT / `## HANDOFF` **交由 driver 追加**进 `STATE.Decisions log`——守 §6 STATE 单写者,不自行写 STATE),**下一轮迭代开始先读本叶教训栈**再动手——让每次失败成为下次的输入,而非白费。
...
- **入既有载体**:自生成测试加入 **repo 正常测试集** + 记入 `.sdlc/validate/correctness-report.md` 回归项,**不是一次性抛弃、不新建载体**。
- **时序(关键)**:这些测试在 **build/validate 阶段生成并跑绿(在 review 之前)**,使 review 覆盖到它们;converge(§4)只**核对**、不在 converge 阶段新增改动。**若 converge 后产生任何 repo diff(补测/补实现)→ 必须回 validate + review 刷新 `sdlc-gate reviewed-head`**,绝不带过期 gate 直接 ship。
